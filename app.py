...
@app.route('/via_naman', methods=['POST']) # for calling the API from Postman/SOAPUI
...
def url_test():
    ...
    return "<h1> myresult is :{} , {} </h1>".format(test1,test2)
@app.route("/add",methods=['GET'])
def url():
    test1=int(request.args.get('val1'))
    test2=int(request.args.get("val2"))
    test3=test1+test2
    return "<h1> myresult is :{} </h1>".format(test3)
